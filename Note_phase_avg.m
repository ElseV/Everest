...
for i = 1%:7
n=1;  
name=string("AVG"); % AVG_Seq_all.seq%d
% A1=i;
% part1=char(sprintf(name,A1));
seq_ref=eval(name);%(part1);

position_ref=pos_aurora_avg(seq_ref,1,21); % avg or normal?
Reference = position_ref;

    for ii=1:15
        name2=('data0724_list.seq%d');
        B1=ii;
        part2=char(sprintf(name2,B1));
        seq_measure=eval(part2);

        position_measure=pos_aurora(seq_measure,1,21); % avg or normal?
        Measurement = note_phase(position_measure);

        [~,ix,iy]=dtw(Reference',Measurement(:,1:end-2)'); % also (:,1:end-2) for Ref
        DTWref= Reference(ix,:);
...
%% add phases to avgavg
AVGavg_aurora=[AVG cell2mat(maj_phases{1}')];
AVGavg_aurora(:,1)=[];
%% add phases
